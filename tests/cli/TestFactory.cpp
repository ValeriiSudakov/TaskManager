...
TEST_F(TestFactory, shouldCorrectCreateMenu){
  ...
  auto baseMenu = Factory::CreateMenuStateMachine(StatesID::BASE_MENU, context, std::move(io));
  ASSERT_NE(baseMenu, nullptr);

  auto continueMenu = Factory::CreateMenuStateMachine(StatesID::CONTINUE_MENU, context, std::move(io));
...
TEST_F(TestFactory, shouldCorrectCreate){
  ...
  auto inputName = Factory::CreateFiniteStatesMachine(FiniteStateMachineID::INPUT_NAME, context, std::move(io));
  ASSERT_NE(inputName, nullptr);

  auto inputLabel = Factory::CreateFiniteStatesMachine(FiniteStateMachineID::INPUT_LABEL, context, std::move(io));
  ASSERT_NE(inputName, nullptr);

  auto inputID = Factory::CreateFiniteStatesMachine(FiniteStateMachineID::INPUT_ID, context, std::move(io));
  ASSERT_NE(inputID, nullptr);

  auto inputDate = Factory::CreateFiniteStatesMachine(FiniteStateMachineID::INPUT_DATE, context, std::move(io));
  ASSERT_NE(inputDate, nullptr);

  auto postpone = Factory::CreateFiniteStatesMachine(FiniteStateMachineID::POSTPONE, context, std::move(io));
  ASSERT_NE(postpone, nullptr);

  auto taskParams = Factory::CreateFiniteStatesMachine(FiniteStateMachineID::INPUT_TASK_PARAMS, context, std::move(io));
  ASSERT_NE(taskParams, nullptr);

  auto subtaskParams = Factory::CreateFiniteStatesMachine(FiniteStateMachineID::INPUT_SUBTASK_PARAMS, context, std::move(io));
